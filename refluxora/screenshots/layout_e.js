...
window.RefluxoraLayout=function(f){
 ...
 if(f.position==='02'){
  ...
  art.querySelector('.art-brand')?.remove();window.RefluxoraSetText(art.querySelector('.app-name'),f.ui.title,f.dir);return art;
 }
 const copy=f.copy||{};const art=n('div',`art series-E e-${f.position} locale-${f.locale||'en-US'}`);art.dataset.id=f.id;art.lang=f.lang||'en';art.dir=f.dir||'ltr';
 ...
 if(f.position==='01')art.append(n('div','e-anatomy-caption',copy.anatomyCaption));
 if(f.report){
  ...
  const finding=n('div','e-report-finding');finding.append(n('span','',f.report.findingLabel+':'),n('strong','e-paper-term',f.report.finding));paper.append(finding,n('div','e-report-rule'),n('span','e-paper-note',copy.paperNote));art.append(paper,n('div','e-report-connector',copy.reportConnector));
 }
 const app=n('div','e-app'),top=n('div','e-app-top');top.append(n('strong','',f.ui.title),n('span','',copy.example));app.append(top);
 ...
 if(f.report){const finding=n('div','e-ui-finding');finding.append(n('span','',copy.reportLead),n('strong','e-ui-term',f.report.finding));app.append(finding);}
