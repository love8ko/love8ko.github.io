...
window.RefluxoraLayout=function(f){
 ...
 if(f.position==='02'){
  ...
  const photo=art.querySelector('.art-photo'),scene=n('div','e-restaurant-window');photo.replaceWith(scene);scene.append(photo);return art;
 }
 ...
 if(f.report){
  const paper=n('div','e-report-print');paper.append(n('strong','e-report-title',f.report.title),n('span','e-report-demo',f.report.date),n('div','e-report-rule'));
  const finding=n('div','e-report-finding');finding.append(n('span','',f.report.findingLabel+':'),n('strong','e-paper-term',f.report.finding));paper.append(finding,n('div','e-report-rule'),n('span','e-paper-note','A finding written in your report'));art.append(paper,n('div','e-report-connector','↘'));
 }
 const app=n('div','e-app'),top=n('div','e-app-top');top.append(n('strong','','Refluxora'),n('span','','AI explanation · Example'));app.append(top);
