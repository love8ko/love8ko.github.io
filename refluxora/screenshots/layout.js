...
window.RefluxoraSetText=function(element,text,direction){
 element.replaceChildren();if(text===undefined||text===null||text==='')return element;
 if(direction!=='rtl'){element.textContent=String(text);return element;}
 const parts=String(text).split(/((?:PDF|AI|NIDDK|Refluxora)|(?:\d[\d:.,%/\-–—]*\d|\d))/giu);
 for(const part of parts){if(!part)continue;if(/^(?:PDF|AI|NIDDK|Refluxora|\d)/iu.test(part)){const bdi=document.createElement('bdi');bdi.dir='ltr';bdi.textContent=part;element.append(bdi);}else element.append(document.createTextNode(part));}
 return element;
};
window.RefluxoraLayout=function(f){
 const n=(tag,cls,text)=>{const e=document.createElement(tag);e.className=cls;if(text)window.RefluxoraSetText(e,text,f.dir);return e;};
 const copy=f.copy||{};
 ...
 const dish=cls=>{const e=n('img',cls);e.src=f.foodAsset;e.alt=copy.dishAlt;return e;};
 const art=n('div',`art human casting-${f.series} frame-${f.position} locale-${f.locale||'en-US'}`);art.dataset.id=f.id;art.lang=f.lang||'en';art.dir=f.dir||'ltr';
 ...
 const ui=n('div','ui-body'),top=n('div','ui-top');top.append(n('span','ui-back','‹'),n('span','app-name',f.ui.title),n('span','ui-menu','···'));ui.append(top,n('div','ui-label',f.ui.label));
 if(f.ui.question)ui.append(n('div','question-bubble',f.ui.question));
 if(f.foodAsset){const input=n('div','dish-input');input.append(n('h4','',copy.dishTitle),n('p','',copy.dishDescription),dish('ui-dish'));ui.append(input);}
 ...
 if(f.source)ui.append(n('div','ui-source',copy.sourceLabel));
 ...
 if(f.foodAsset){const food=n('div','food-popout');food.append(dish('hero-dish'),n('span','food-caption',copy.dishCaption));art.append(food);}
 if(f.position==='08'){const summary=f.summary;const doc=n('div','summary-popout');doc.append(n('span','doc-brand',summary.format),n('strong','',summary.title),n('span','summary-dates',summary.date));for(const t of summary.rows)doc.append(n('div','summary-line','✓ '+t));art.append(doc);}
 ...
 const foot=n('div','art-foot');foot.append(n('span','',copy.footer),n('span','',f.id));art.append(foot);return art;
};

/* Deterministic fit for translated display copy. Applied before every export. */
window.RefluxoraFitText=function(root=document){
 const fit=(el,maxLines,minPx)=>{if(!el)return;const line=()=>parseFloat(getComputedStyle(el).lineHeight);let size=parseFloat(getComputedStyle(el).fontSize);while(el.getBoundingClientRect().height>line()*maxLines+2&&size>minPx){size-=1;el.style.fontSize=size+'px';}};
 fit(root.querySelector('.art-head h3'),3,38);
 fit(root.querySelector('.art-subline'),2,24);
 root.querySelectorAll('.proof-focus,.e-app .ui-focus').forEach(el=>fit(el,3,21));
};
