...
window.RefluxoraLayout=function(f){
 ...
 const art=n('div',`art human casting-${f.series} frame-${f.position}`);art.dataset.id=f.id;
 ...
 const ui=n('div','ui-body'),top=n('div','ui-top');top.append(n('span','ui-back','‹'),n('span','app-name','Refluxora'),n('span','ui-menu','···'));ui.append(top,n('div','ui-label',f.ui.label));
 if(f.ui.question)ui.append(n('div','question-bubble',f.ui.question));
 if(f.foodAsset){const input=n('div','dish-input');input.append(n('h4','','Grilled salmon'),n('p','','Lemon butter sauce'),dish('ui-dish'));ui.append(input);}
 ...
 if(f.source)ui.append(n('div','ui-source',f.metric?'↗ About pH monitoring':'↗ NIDDK · Reflux guidance'));
 ...
 if(f.foodAsset){const food=n('div','food-popout');food.append(dish('hero-dish'),n('span','food-caption','EXAMPLE ORDER'));art.append(food);}
 if(f.position==='08'){const doc=n('div','summary-popout');doc.append(n('span','doc-brand','Refluxora · PDF'),n('strong','','Your observations'),n('span','summary-dates','Sep 7–10 · Example'));for(const t of ['Meals & symptoms','Throat & voice notes','Your confirmed report'])doc.append(n('div','summary-line','✓ '+t));art.append(doc);}
